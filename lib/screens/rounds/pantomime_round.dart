...
class PantomimeRound extends StatefulWidget {
  @override
  _PantomimeRoundState createState() => _PantomimeRoundState();
}

class _PantomimeRoundState extends State<PantomimeRound> {
  @override
  void initState() {
    super.initState();

    toPlayInPant = playedInNormal + playedInOne;
  }
  ...
  bool isVisible() {
    if (_start <= 0.0 || _start == 60) return false;
    return true;
  }
              ...
              if (_start <= 0)
                AnimatedButton(
                  ...
                  onPressed: () {
                    ...
                    sharedPref.save(master.teamToPlay.toString(), currentTeam);
                    if (master.teamToPlay != master.numOfTeams) {
                      master.teamToPlay += 1;
                    } else if (currentTeam.playerExplaining == 2) {
                      master.teamToPlay = 1;
                      master.roundToPlay = 3;
                    } else {
                      master.roundToPlay = 4;
                    }
